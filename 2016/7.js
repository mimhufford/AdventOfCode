...
const outsideMatch = /^([a-z]+)|[\]]([a-z]+)[\[]|([a-z]+)$/g
const insideMatch = /[\[]([a-z]+)[\]]/g

const containsAnABBA = text => text
        .split("")
        .slice(0, text.length - 3)
        .map((l, i) => [l, text[i + 1], text[i + 2], text[i + 3]])
        .filter(c => c.join("") === c.reverse().join(""))
        .some(c => c[0] !== c[1])

const addresses = input.split("\n").map(a => {
    return {
        address: a,
        outside: a.match(outsideMatch).map(i => i.replace(/[\[\]]/g, "")),
        inside: a.match(insideMatch).map(i => i.replace(/[\[\]]/g, ""))
    }
})

const supportsTLS = addresses.filter(a => a.outside.some(containsAnABBA) && a.inside.some(containsAnABBA) === false).length
