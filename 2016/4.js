...
const parseInfo = data => {
    ...
    return { encName: parts[1], sectorId: parseInt(parts[2]), checksum: parts[3] }
}

const calculateChecksum = name => {
    const letters = name.replace(/-/g, "")
    const alphabet = "abcdefghijklmnopqrstuvwxyz".split("")
    const letterFreqs = alphabet.map(c1 => [c1, letters.split("").filter(c2 => c1 === c2).length])
    // sort by freq, and within that alphabetically
    const sortedFreqs = letterFreqs.sort((a, b) => a[1] > b[1] ? -1 : a[1] < b[1] ? 1 : a[0] < b[0] ? -1 : 1)
    const letterOrder = sortedFreqs.map(sf => sf[0]).join("")
