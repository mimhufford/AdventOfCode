...
const nodes = input.trim().split("\n").map(line => {
    const n = line.match(/\/dev\/grid\/node-x(\d+)-y(\d+)\s+(\d+)T\s+(\d+)T\s+(\d+)T\s+(\d+)%/).slice(1).map(Number)
    return {
        id: n[0] + ":" + n[1],
        x: n[0],
        y: n[1],
...
const pairs = nodes.filter(node => node.used > 0).map(A => { return { A, pairs: nodes.filter(B => A.id !== B.id && A.used <= B.available) } })

console.log(pairs.map(node => node.pairs.length).reduce((a, b) => a + b))