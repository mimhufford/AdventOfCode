...
const generateGrid = (rows, cols) => new Array(rows).join(".").split(".").map(i => new Array(cols + 1).join(".").split(""))
const display = grid => grid.map(row => row.join("")).join("\n")

const rot = (amount, data) => [...data.slice(data.length - amount, data.length), ...data.slice(0, data.length - amount)]
const drwRec = (width, height) => grid => grid.map((row, ri) => ri < height ? row.map((cell, ci) => ci < width ? "#" : cell) : row)
...
const generateInstruction = ins => {
    ...
    if (recRes) return drwRec(parseInt(recRes[1]), parseInt(recRes[2]))
    if (rowRes) return rotRow(parseInt(rowRes[1]), parseInt(rowRes[2]))
